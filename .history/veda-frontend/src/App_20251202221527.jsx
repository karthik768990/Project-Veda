...
const Generator = () => {
  ...
  const [chandas, setChandas] = useState(''); // Init empty, will fill from DB
    ...
    fetch(`${API_BASE}/chandas`)
      ...
      .then(data => {
        if(data.success && data.data.length > 0) {
          ...
          // Auto-select the first valid chandas
  ...
  const getDisplayShloka = () => {
    if (!out) return "";
    if (out.success && out.final?.shloka) return out.final.shloka;
    if (!out.success && out.attempts && out.attempts.length > 0) {
      return out.attempts[out.attempts.length - 1].shloka || "Generation Error";
    }
  ...
  const getAnalysisStatus = () => {
    if (!out) return null;
    if (out.success) return { text: "Verified Match", color: "#4ade80", icon: <CheckCircle size={14}/> };
    const lastAttempt = out.attempts && out.attempts.length > 0 ? out.attempts[out.attempts.length - 1] : null;
    const detected = lastAttempt?.analysis?.identifiedChandas || "Unknown";
        ...
        {out && (
          <div className="result-card glass-panel animate-fadeIn">
            ...
            <div style={{ marginTop: '2rem', paddingTop: '1rem', borderTop: '1px solid var(--border-glass)', display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
              ...
              <div className="verified-badge" style={{ color: status?.color, borderColor: status?.color, background: status?.color + '20' }}>
