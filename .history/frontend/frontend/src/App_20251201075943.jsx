...
import { 
  ...
  Menu, X, Search, Zap, BookOpen, PenTool, CheckCircle, AlertCircle, Loader2, Sparkles 
} from 'lucide-react';
...
// --- 1. BACKGROUND COMPONENT (Floating Scripts) ---
const FloatingScripts = () => {
  ...
  // Memoize particle generation to prevent re-renders
  const floatingElements = useMemo(() => {
    return Array.from({ length: 40 }).map((_, i) => ({
      id: i,
      char: scripts[Math.floor(Math.random() * scripts.length)],
      left: `${Math.random() * 100}%`,
      animationDuration: `${15 + Math.random() * 25}s`, // Varied speeds
      delay: `${Math.random() * -30}s`, // Negative delay for instant start
        ...
        <div className="mt-10 p-8 bg-black/40 border border-orange-500/30 rounded-xl relative z-10 animate-fadeIn">
          ...
          <div className="relative">
            ...
            <pre className="text-2xl md:text-3xl text-white font-serif text-center whitespace-pre-wrap leading-relaxed drop-shadow-lg">
              {generated.shloka || "Generation failed."}
            </pre>
          ...
          {generated.analysis && (
             <div className="mt-8 pt-6 border-t border-white/10 grid grid-cols-2 gap-4 text-xs text-slate-400">
                <div>
                   ...
                   <span className="text-slate-300 font-mono">{JSON.stringify(generated.analysis.identifiedChandas)}</span>
                </div>
