...
// --- CONFIGURATION ---
// Hardcoded defaults to ensure compatibility with ES2015 build targets
const API_BASE = "http://localhost:3000";
// TODO: PASTE YOUR GOOGLE CLIENT ID HERE
const GOOGLE_CLIENT_ID = "YOUR_GOOGLE_CLIENT_ID_HERE"; 

// --- 1. AUTH GATEKEEPER COMPONENT (New) ---
const AuthGate = ({ onLogin }) => {
  useEffect(() => {
    // Check if Client ID is configured
    if (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID.includes("YOUR_GOOGLE_CLIENT_ID")) {
      console.error("Missing Google Client ID");
      // We don't alert here to avoid spamming, but the button won't render correctly without a valid ID
    }
    ...
    script.onload = () => {
      ...
      // Only initialize if we have a potentially valid ID
      if (GOOGLE_CLIENT_ID && !GOOGLE_CLIENT_ID.includes("YOUR_GOOGLE_CLIENT_ID")) {
  ...
  const verifyToken = async (token) => {
    try {
      ...
      console.error(e);
      alert("Backend connection failed. Is FastAPI running on port 3000?");
    }
  };
      ...
      <div className="p-10 border border-orange-500/30 rounded-2xl bg-slate-950/90 flex flex-col items-center gap-6 shadow-2xl max-w-md w-full relative overflow-hidden">
        ...
        <div id="google-btn" className="flex justify-center min-h-[44px]">
           {(!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID.includes("YOUR_GOOGLE_CLIENT_ID")) && (
             <span className="text-red-500 text-xs bg-red-900/20 px-3 py-2 rounded border border-red-500/30">
               Error: Add Client ID in App.jsx
             </span>
           )}
        </div>
...
// --- 2. BACKGROUND COMPONENT (Floating Scripts) ---
const FloatingScripts = () => {
  const scripts = [
    ...
    'ट', 'ठ', 'ड', 'ढ', 'ण', 'त', 'थ', 'द', 'ध', 'न',
    'प', 'फ', 'ब', 'भ', 'म', 'य', 'र', 'ल', 'व', 'श',
    'ष', 'स', 'ह', 'क्ष', 'त्र', 'ज्ञ', 'ॐ', 'श्री', 'शान्ति',
    'विद्या', 'ज्ञान', 'कर्म', 'धर्म', 'सत्य', 'प्रेम', 'अहिंसा',
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0 bg-slate-950">
      ...
      {floatingElements.map((el) => (
        <div
          key={el.id}
          className="absolute text-orange-200/40 font-serif select-none animate-float blur-[1px]"
          style={{
            left: el.left,
...
const fetchAPI = async (endpoint, method = 'GET', body = null) => {
  try {
    ...
    throw new Error('Connection refused. Is the backend running?');
  }
};
