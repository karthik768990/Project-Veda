...
const Generator = () => {
  ...
  // --- SAFE PARSING LOGIC (CRASH PROOF) ---
  const getDisplayShloka = () => {
    if (!out) return "";
    
    // Safely check for final object
    ...
    // Safely check for attempts array
  ...
  const getAnalysisStatus = () => {
    ...
    // Safely determine which object holds the analysis data
    // Use optional chaining (?.) everywhere to prevent crashes
    const finalData = out.final || (out.attempts && out.attempts.length > 0 ? out.attempts[out.attempts.length - 1] : {});
    
    // Check keys: 'match' (from backend) or 'analysis' (legacy)
